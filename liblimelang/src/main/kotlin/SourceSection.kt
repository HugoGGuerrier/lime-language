...
data class SourceSection(val source: Source, val start: SourceLocation, val end: SourceLocation) {
    companion object {
        ...
        fun fromPosition(source: Source, position: Position): SourceSection =
            SourceSection(
                source,
                SourceLocation(position.start.line, position.start.column),
                SourceLocation(position.end.line, position.end.column)
            )
    }
